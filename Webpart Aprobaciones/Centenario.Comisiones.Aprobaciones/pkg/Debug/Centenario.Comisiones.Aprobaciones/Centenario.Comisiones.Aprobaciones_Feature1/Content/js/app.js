...
var App = function() {
	...
	$.ajax({
		...
		success: function (data, textStatus, xhr) {
			...
			var nombre = dataResults.DisplayName;
			$('.name').text(nombre);
	...
	$.ajax({
		...
		success: function (data, textStatus, xhr) {
            ...
            var username = GetPropertyVal(properties, "UserName");
			var foto = username.split("@")[0];
			//$('.avatar-status').html("<img src='https://eur.delve.office.com/mt/v3/people/profileimage?userId="+foto+"%40engie.com&size=L'/>");
			$('.avatar-status').html("<img src='https://outlook.office.com/owa/service.svc/s/GetPersonaPhoto?email="+foto+"%40centenario.com.pe&amp;UA=0&amp;size=HR96x96'/>");
            var cargo = GetPropertyVal(properties, "SPS-JobTitle");
			$('.cargo').text(cargo);
            var dpto = GetPropertyVal(properties, "Office");
			$('.dpto').text(dpto);
            var fono = GetPropertyVal(properties, "WorkPhone");
			$('.fono').text(fono);
    ...
    var uiLayoutApi = function($mode) {
        ...
        switch($mode) {
            ...
            case 'menu_overlay_toggle':
                $lPage.toggleClass('menu-overlay-o');
                break;
            case 'menu_overlay_open':
                $lPage.addClass('menu-overlay-o');
                break;
            case 'menu_overlay_close':
                $lPage.removeClass('menu-overlay-o');
                break;
