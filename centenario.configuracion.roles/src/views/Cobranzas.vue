...
export default {
  ...
  created(){
    API.getListItems(SUBSITIOS.Cobranzas, LISTAS.EJECUTIVOS_COBRANZAS, null, ['Ejecutivo/Id', 'Ejecutivo/Title', 'Rol/Codigo', 'Rol/Title'], ['Ejecutivo', 'Rol']).then( ejecutivosurbanizaciones => {
        this.ejecutivos = ejecutivosurbanizaciones;
    }).then(() => {
        API.getListItems(null, LISTAS.CONFIGURACIONACCESOS, {SubSitio: CONSTANTES.CREDITOS_COBRANZAS}, null).then( configuracionAccesos => {
            let idUsuario = this.$store.state.currentUser.Id;
            this.ejecutivos = this.ejecutivos.filter(x => x.Ejecutivo.Id == idUsuario);
            let confAccesos =configuracionAccesos.filter(x => x.SubSitio == CONSTANTES.CREDITOS_COBRANZAS);
            let confAccesostemp = [];
    ...
    API.getListItems(SUBSITIOS.Cobranzas, LISTAS.ROLES, null, ['Id', 'Title']).then( roles => {
        this.roles = roles;            
    })
  },
  methods:{
