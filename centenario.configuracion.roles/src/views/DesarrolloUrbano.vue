...
export default {
  ...
  created(){
    API.getListItems(SUBSITIOS.Desarrollo_Urbano, LISTAS.EJECUTIVOS_URBANIZACIONES, null, ['Ejecutivo/Id', 'Ejecutivo/Title', 'Rol/Codigo', 'Rol/Title'], ['Ejecutivo', 'Rol']).then( ejecutivosurbanizaciones => {
        this.ejecutivos = ejecutivosurbanizaciones;
        API.getListItems(SUBSITIOS.Desarrollo_Urbano, LISTAS.EJECUTIVOS_INDUSTRIAL, null, ['Ejecutivo/Id', 'Ejecutivo/Title', 'Rol/Codigo', 'Rol/Title'], ['Ejecutivo', 'Rol']).then( ejecutivosindustrial => {
            if(this.ejecutivos.length > 0)
            {
                let ejecutivosUnidos = this.ejecutivos.concat(ejecutivosindustrial);
                this.ejecutivos = ejecutivosUnidos;
            }
        })
    }).then(() => {
        API.getListItems(null, LISTAS.CONFIGURACIONACCESOS, {SubSitio: CONSTANTES.DESARROLLO_URBANO}, null).then( configuracionAccesos => {
            let idUsuario = this.$store.state.currentUser.Id;
            this.ejecutivos = this.ejecutivos.filter(x => x.Ejecutivo.Id == idUsuario);
            let confAccesos =configuracionAccesos.filter(x => x.SubSitio == CONSTANTES.DESARROLLO_URBANO);
            let confAccesostemp = [];
    ...
    API.getListItems(SUBSITIOS.Desarrollo_Urbano, LISTAS.ROLES, null, ['Id', 'Title']).then( roles => {
        this.roles = roles;            
    })
  },
  methods:{
